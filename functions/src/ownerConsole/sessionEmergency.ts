...
function trustedRecord(
  ...
  return {
    ...
    order: data.order,
    publicationStatus:
      id === withdrawnSessionId ? "draft" : data.publicationStatus,
...
function deriveDiscovery(
  ...
  if (Number.isNaN(trustedNow.getTime()))
    throw new Error("Trusted emergency time is invalid.");
  const records = sessions.map((snapshot) =>
    trustedRecord(snapshot.id, snapshot.data(), withdrawnSessionId),
  );
  if (!records.some((record) => record.id === withdrawnSessionId))
...
function releaseState(
  ...
  trustedNow: Date,
): "immediate" | "released" | "scheduled" {
  if (!Object.prototype.hasOwnProperty.call(session, "releaseAt"))
    return "immediate";
  return (session.releaseAt as Timestamp).toMillis() <= trustedNow.getTime()
    ? "released"
    : "scheduled";
}
...
export async function reviewSessionEmergencyWithdrawal(
  ...
  const resourcesQuery = sessionRef
    .collection("resources")
    .limit(SESSION_EMERGENCY_RESOURCE_LIMIT + 1);
  const [course, module, session, sessions, visible, free, resources] =
    await Promise.all([
      courseRef.get(),
      moduleRef.get(),
      sessionRef.get(),
      sessionsQuery.get(),
      db.doc(selected.visible).get(),
      db.doc(selected.free).get(),
      resourcesQuery.get(),
    ]);
  if (!course.exists || !module.exists || !session.exists)
    throw new Error("Session hierarchy was not found.");
  ...
  if (current.publicationStatus !== "published")
    throw new Error("Only a published Session can be withdrawn.");
  ...
  if (visible.exists && typeof visible.data() !== "object")
    throw new Error("Visible discovery is malformed.");
  if (free.exists && typeof free.data() !== "object")
    throw new Error("Free discovery is malformed.");
  void proposed;
  return {
    operation: "session-emergency-withdrawal",
    target,
    ...
    safe: {
      ...
      bindingsPreserved: true,
      warning:
        "This removes the Session from student access and discovery. Video/PDF bindings and encrypted files remain for recovery. Already acquired content cannot be revoked.",
    },
  };
}
...
async function verifyCommittedState(
  ...
  const [session, sessions, visible, free] = await Promise.all([
    db.doc(selected.session).get(),
    moduleRef.collection("sessions").get(),
    db.doc(selected.visible).get(),
    db.doc(selected.free).get(),
  ]);
  if (!session.exists || !visible.exists || !free.exists)
  ...
  if (
    !sessionDiscoveryManifestsEqual(visible.data(), expected.visible) ||
...
export async function applySessionEmergencyWithdrawal(
  ...
  if (review.operation !== "session-emergency-withdrawal")
    throw new Error("Emergency review operation is invalid.");
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (
      !visible.exists ||
      !sessionDiscoveryManifestsEqual(visible.data(), proposed.visible)
    )
      transaction.set(visibleRef, {
        sessionIds: [...proposed.visible.sessionIds],
      });
    if (
      !free.exists ||
      !freeSessionDiscoveryManifestsEqual(free.data(), proposed.free)
    )
      transaction.set(freeRef, {
        sessions: proposed.free.sessions.map((item) => ({ ...item })),
      });
  });
  try {
    await (
      dependencies.verifyCommitted ??
      (() => verifyCommittedState(db, target, trustedNow))
    )();
    return {
      status: "COMMITTED_AND_VERIFIED" as const,
      postApplyVerified: true,
    };
  } catch {
    return {
      status: "COMMITTED_VERIFICATION_UNCERTAIN" as const,
