...
export const FREE_STATUS_PUBLISHED_CONFIRMATION = "CHANGE PUBLISHED SESSION ACCESS";
...
function trustedRecord(snapshot: { id: string; data(): DocumentData }) {
  ...
  return {
    ...
    ...(Object.prototype.hasOwnProperty.call(data, "releaseAt")
      ? { releaseAt: data.releaseAt instanceof Timestamp ? data.releaseAt.toDate() : null }
      : {}),
  };
}
...
export async function reviewSessionFreeStatus(
  ...
  if (typeof proposedIsFree !== "boolean") throw new Error("Free status is invalid.");
  const target = {
  ...
  const moduleRef = db.doc(`courses/${target.courseId}/modules/${target.moduleId}`);
  const sessionRef = moduleRef.collection("sessions").doc(target.sessionId);
  const [course, module, session] = await db.getAll(courseRef, moduleRef, sessionRef);
  if (!course.exists || !module.exists || !session.exists) throw new Error("Session was not found.");
...
export async function applySessionFreeStatus(
  ...
  if (Number.isNaN(trustedNow.getTime())) throw new Error("Trusted time is invalid.");
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (session.updateTime?.toMillis() !== review.revisionMillis || (current.isFree === true) !== review.currentIsFree) {
    ...
    const records = sessions.docs.map((item) => {
      ...
      return item.id === sessionId ? { ...record, isFree: review.proposedIsFree } : record;
    });
    const manifest = buildFreeSessionDiscoveryManifest(records, trustedNow);
    transaction.set(freeRef, { sessions: manifest.sessions.map((item) => ({ ...item })) });
  });
  const verified = validateSessionForVideoPublication((await sessionRef.get()).data());
  if ((verified.isFree === true) !== review.proposedIsFree) throw new Error("Free status verification failed.");
